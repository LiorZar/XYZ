...
int main()
{
    ...
    Buffer<float> curr1[2], prev1[2], out1[2];
    ...
    int t0 = program.Dispatch1D("Transpose2DPalanar", num_of_samples, GRP, *curr, *curr1[0], *curr1[1], num_of_channels, samples_per_channel);
    int t1 = program.Dispatch1D("Transpose2DPalanar", num_of_samples, GRP, *prev, *prev1[0], *prev1[1], num_of_channels, samples_per_channel);
    ...
    int t2 = program.Dispatch1D("convolve1D", num_of_samples, GRP, *prev1[0], *curr1[0], *filter1, *out1[0], num_of_samples, filter_size, num_of_channels);
    int t3 = program.Dispatch1D("convolve1D", num_of_samples, GRP, *prev1[1], *curr1[1], *filter1, *out1[1], num_of_samples, filter_size, num_of_channels);
    el.Stamp("convolve1D");
    out1[0].Download();
    out1[1].Download();

    int t4 = program.Dispatch1D("InverseTranspose2DPalanar", num_of_samples, GRP, *out1[0], *out1[1], *out, samples_per_channel, num_of_channels);
